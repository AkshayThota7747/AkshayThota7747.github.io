const roosterMap =
{
  kodi_kaki: {
    'url': '/assets/kodi_kaki_pingala_chupu.jpeg',
    'text': 'కోడి కాకి'
  }

  , sethuva: {
    'url': '/assets/sethuva.jpeg',
    'text': 'సేతువా'
  }

  , kodi_pacha_kaki: {
    'url': '/assets/kodi_chupu_pasimi_gala_pacha_kaki.webp',
    'text': 'కోడి పచ్చ కాకి'
  }

  , thella_kodi_kekkirayi: {
    'url': '/assets/kekkirayi.jpeg',
    'text': 'తెల్ల కోడి కొక్కిరాయి'
  }

  , kodi_rasangi: {
    'url': '/assets/rasangi.jpeg',
    'text': "కోడి రసంగి"
  }

  , kattu_kaki: {
    'url': '/assets/kattu_kaki.jpeg',
    'text': 'కాకి'
  }

  , kaki_dega_parla: {
    'url': '/assets/erra_nemali.jpeg',
    'text': 'కాకి డేగ పర్ల'
  }

  , erupu_minchina_getti_kaki: {
    'url': '/assets/kaki_dega.jpeg',
    'text': 'ఎరుపు మించిన గెట్టి కాకి డేగ'
  }

  , erp_get_kaki_dega: {
    'url': '/assets/erp_get_kaki_dega.jpeg',
    'text': 'ఎరుపు మించిన పచ్చ కాకి డేగ'
  }

  , nemali_pingala: {
    'url': '/assets/pingala.jpeg',
    'text': 'నెమలి పింగళ'
  }

  , thella_nemali: {
    'url': '/assets/thella_savala.webp',
    'text': 'తెల్ల నెమలి'
  }

  , pala_abrass: {
    'url': '/assets/pala_abrass.jpeg',
    'text': 'పాల అబ్రసు'
  }

  , nemali_poola: {
    'url': '/assets/nemali_poola.jpeg',
    'text': 'నెమలి పూల'
  }

  , kodi_dega: {
    'url': '/assets/kodi_dega.jpeg',
    'text': 'కోడి డేగ'
  }

  , errabotlu: {
    'url': '/assets/nallabotlu_errabotlu.webp',
    'text': 'ఎర్రబొట్ల సేతువా'
  }

  , erra_kodi_kekkirayi: {
    'url': '/assets/gaju_kekkirayi.jpeg',
    'text': 'ఎర్ర కోడి కొక్కిరాయి'
  }

  , erupu_kaki_nemali: {
    'url': '/assets/erupu_minchina_kaki_nemali.jpeg',
    'text': 'ఎరుపు మించిన కాకి నెమలి'
  }

  , pacha_kaki: {
    'url': '/assets/pacha_kaki.jpeg',
    'text': 'పచ్చ కాకి'
  }

  , pacha_kaki_org: {
    'url': '/assets/pacha_kaki_2.webp',
    'text': 'పచ్చ కాకి'
  }

  , nemali_maila: {
    'url': '/assets/nemali_maila_new.jpeg',
    'text': 'నెమలి మైల'
  }

  , poola_rasangi: {
    'url': '/assets/poola_rasangi.jpeg',
    'text': 'కోడి రసంగి'
  }

  , kodi_pingali: {
    'url': '/assets/kodi_pingala.jpeg',
    'text': 'కోడి పింగళ'
  }
  , kodi_pingali_2: {
    'url': '/assets/kodi_pingala_2.jpeg',
    'text': 'కోడి పింగళ'
  }

  , erra_kaki_dega: {
    'url': '/assets/erra_kaki_dega_new.jpeg',
    'text': 'ఎర్ర కాకి డేగ'
  }

  , kaki: {
    'url': '/assets/kaki_dega_new.jpeg',
    'text': 'కాకి డేగ'
  }

  , np_abrass: {
    'url': '/assets/nemali_pingala_abrass.jpeg',
    'text': 'నెమలి పింగళ అబ్రసు'
  }

  , dega: {
    'url': '/assets/dega.jpeg',
    'text': 'డేగ'
  }

  , kaki_nemali: {
    'url': '/assets/kaki_nemali_new.jpeg',
    'text': 'కాకి నెమలి'
  }

  , nemali_daga: {
    'url': '/assets/nemali_dega.webp',
    'text': 'నెమలి డేగ'
  }

  , nalla_savala: {
    'url': '/assets/nalla_savala_2.jpeg',
    'text': 'నల్ల సవల'
  }

  , kodi_chupu_kn: {
    'url': '/assets/kodi_chupu_kaki_nemali_or_kodi_kaki.jpeg',
    'text': 'కోడి కాకి నెమలి'
  }

  , kodi_chupu_kd: {
    'url': '/assets/kodi_chupu_kaki_dega.jpeg',
    'text': 'కోడి చూపు కాకి డేగ'
  }

  , nbs: {
    'url': '/assets/nallabotla_sethuva.webp',
    'text': 'నల్లబొట్ల సేతువ'
  }

  , budidha_rangu_maila: {
    'url': '/assets/budidha_rangu_maila.webp',
    'text': 'నల్ల కోడి చూపు మైల'
  }

  , black_kekkira: {
    'url': '/assets/nalla_kekkirayi.webp',
    'text': 'నల్ల కోడి కొక్కిరాయి'
  }

  , bk: {
    'url': '/assets/nalla_kekkira.jpeg',
    'text': 'నల్ల కొక్కిరాయి'
  }

  , kodi_nemili: {
    'url': '/assets/kodi_nemali_3.jpeg',
    'text': 'కోడి నెమలి'
  }

  , white_kekkira: {
    'url': '/assets/thella_kekkirayi.jpeg',
    'text': 'తెల్ల కోడి కొక్కిరాయి'
  }

  , goudu_nemali: {
    'url': '/assets/pichukupoda_nemali.jpeg',
    'text': 'గౌడు నెమలి'
  }

  , pettamari_k: {
    'url': '/assets/kaki_pettamari2.webp',
    'text': 'కాకి పెట్టమరి'
  }

  , pacha_pettemaru: {
    'url': '/assets/pacha_kaki_pettamari.jpeg',
    'text': 'పచ్చ కాకి పెట్టమరి'
  }

  , kaaaki: {
    'url': '/assets/kaki.jpeg',
    'text': 'కాకి'
  }

  , n_d: {
    'url': '/assets/nemali_dega.jpeg',
    'text': 'నెమలి డేగ'
  }

  , er_maila: {
    'url': '/assets/erra_maila.jpeg',
    'text': 'ఎర్ర మైల'
  }

  , n_abras: {
    'url': '/assets/nemali_abrass.jpeg',
    'text': 'నెమలి అబ్రసు'
  }

  , org_deg: {
    'url': '/assets/dega_original.jpeg',
    'text': 'డేగ'
  }

  , erra_kekkirai: {
    'url': '/assets/erra_kekkirayi.jpeg',
    'text': 'ఎర్ర కొక్కిరాయి'
  }

  , geruva: {
    'url': '/assets/geruva.png',
    'text': 'గెరువ'
  }

  , t_bunga_p_k: {
    'url': '/assets/kodi_chupu_pasimi_gala_pacha_kaki.webp',
    'text': 'తెల్ల బుంగ పచ్చ కాకి'
  }
}

function getNameInfo(name) {
  return roosterMap[name] || roosterMap['t_bunga_p_k'];
}



// 1. kodi_pingala_jamu
// 2. kaki_dega_jamu
// 3. nemali_pingala_jamu
// 4. kodi_dega_jamu
// 5. kodi_kaki_jamu
// 6. kodi_nemali_jamu
// 7. kaki_pingala_jamu


const kodi_pingala_jamu = 
{
  'header': getTimingText("1") + "🏆 " + getTranslated("kodi_pingala") + " 🏆" + '\n' + 'కోడి పింగళ వంతులో గెలిచే రంగులు ✅',

  "winLakshana": "విన్నింగ్ లక్షణాలు ✅ : లోపల తెల్ల బుంగ ఉండాలి, పైనా పింగళి చూపు ఉన్నవి గెలుచును.",
  
  "losingLakshana": "లాసింగ్ లక్షణాలు ❌ : లోపల నల్లకుసి ఉండాలి, పైనా బిరుసు ఉన్నవి ఓటమి పాలు అవును.",
  
  '1': getNameInfo("sethuva"),

  '2': getNameInfo("thella_kodi_kekkirayi"),

  '3': getNameInfo("poola_rasangi"),

  '4': getNameInfo("kodi_pingali_2"),

  '5': getNameInfo("kodi_pingali"),

  '6': getNameInfo("kodi_pacha_kaki"),

  '7': getNameInfo("kodi_kaki"),
}


const kaki_dega_jamu = {
  'header': getTimingText("2") + "🏆 " + getTranslated("kaki_dega") + " 🏆" + '\n' + getTranslated("kaki_dega") + ' వంతులో గెలిచే రంగులు ✅',

  "winLakshana": "విన్నింగ్ లక్షణాలు ✅ : లోపల నల్లకుసి ఉండాలి, పైనా ఎరుపు ఉన్నవి గెలుచును",
  
  "losingLakshana": "లాసింగ్ లక్షణాలు ❌ : లోపల తెల్లదనం, పైనా మేతదానం ఉన్నవి ఓటమి పాలు అవును",
  
  '1': getNameInfo("kaki"), 

  '2': getNameInfo("kattu_kaki"), 
  
  '3': getNameInfo("erra_kaki_dega"), 

  '4': getNameInfo("erupu_minchina_getti_kaki"),

  '5': getNameInfo("kaki_dega_parla")
}


const nemali_pingala_jamu = {
  'header': getTimingText("3") + "🏆 " + getTranslated("nemali_pingala") + " 🏆" + '\n' + getTranslated("nemali_pingala") + ' వంతులో గెలిచే రంగులు ✅',

  "winLakshana": "విన్నింగ్ లక్షణాలు ✅ : పైనా గెట్టి కుసి ఉండాలి, పైనా బిరుసు ఉండాలి మరియు రంగు గెట్టిది అయ్యి ఉన్నవి గెలుచును.",
  
  "losingLakshana": "లాసింగ్ లక్షణాలు ❌ : నల్ల బుంగ ఉనవి, తెల్ల బుంగ ఉన్నవి ఓటమి పాలు అవును.",

  '1': getNameInfo("nemali_pingala"),

  '2': getNameInfo("np_abrass"),

  '3': getNameInfo("thella_nemali"),

  '4': getNameInfo("pala_abrass"),

  '5': getNameInfo("nemali_poola")
}


const kodi_dega_jamu = {
  'header': getTimingText("4") + "🏆 " + getTranslated("kodi_dega") + " 🏆" + '\n' + getTranslated("kodi_dega") + ' వంతులో గెలిచే రంగులు ✅',

  "winLakshana": "విన్నింగ్ లక్షణాలు ✅ : లోపల తెల్ల బుంగ ఉండాలి, పైనా ఎరుపు ఉన్నవి గెలుచును.",
  
  "losingLakshana": "లాసింగ్ లక్షణాలు ❌ : లోపల నల్ల బుంగ ఉన్నవి ఓటమి పాలు అవును.",

  '1': getNameInfo("kodi_dega"),

  '2': getNameInfo("dega"),
  
  '3': getNameInfo("erp_get_kaki_dega"),
  
  '3': getNameInfo("erra_kodi_kekkirayi"),

  '4': getNameInfo("kodi_rasangi"),
  
  '5': getNameInfo("errabotlu")

}

const kaki_nemali_jamu = {
  'header': getTimingText("5") + "🏆 " + getTranslated("kaki_nemali") + " 🏆" + '\n' + getTranslated("kaki_nemali")+ ' వంతులో గెలిచే రంగులు ✅',

  "winLakshana": "విన్నింగ్ లక్షణాలు ✅ : లోపల నల్ల బొంగ ఉండాలి, పైనా నెమలి చూపు ఉండి బిరుసు ఉన్నవి గెలుచును.",
  
  "losingLakshana": "లాసింగ్ లక్షణాలు ❌ : లోపల తెల్ల బొంగ ఉండాలి, పైనా మేతదానం ఉన్నవి ఓటమి పాలు అవును.",

  '1': getNameInfo("kaki_nemali"),

  '2': getNameInfo("nalla_savala"),

  '3': getNameInfo("n_abras"),

  '4': getNameInfo("nemali_maila"),

  '5': getNameInfo("nemali_daga"),

  '6': getNameInfo("pacha_kaki_org"),
}


const kodi_kaki_jamu = {
  'header': getTimingText("1") + "🏆 " + getTranslated("kodi_kaki") + " 🏆" + '\n' + getTranslated("kodi_kaki")+ ' వంతులో గెలిచే రంగులు ✅',

  "winLakshana": "విన్నింగ్ లక్షణాలు ✅ : లోపల తెల్ల బుంగ ఉండాలి, పైనా బిరుసు ఉన్నవి గెలుచును.",
  
  "losingLakshana": "లాసింగ్ లక్షణాలు ❌ : లోపల గెట్టి కుసి ఉండాలి, పైనా మేతదానం ఉన్నవి ఓటమి పాలు అవును.",

  '1': getNameInfo("kodi_kaki"),

  '2': getNameInfo("kodi_chupu_kn"),

  '3': getNameInfo("black_kekkira"),

  '4': getNameInfo("kodi_pacha_kaki"),

  '5': getNameInfo("nbs"),

  '6': getNameInfo("budidha_rangu_maila"),

  '7': getNameInfo("kodi_chupu_kd"),
  
}


const kodi_nemali_jamu = {
  'header': getTimingText("4") + "🏆 " + getTranslated("kodi_nemali") + " 🏆" + '\n' + getTranslated("kodi_nemali")+ ' వంతులో గెలిచే రంగులు ✅',

  "winLakshana": "విన్నింగ్ లక్షణాలు ✅ : లోపల తెల్ల బుంగ ఉండాలి, పైనా బిరుసు ఉన్నవి గెలుచును.",
  
  "losingLakshana": "లాసింగ్ లక్షణాలు ❌ : లోపల గెట్టి కుసి ఉండాలి, పైనా మేతదనం ఉన్నవి ఓటమి పాలు అవును.",

  '1': getNameInfo("kodi_nemili"),

  '2': getNameInfo("white_kekkira"),

  '3': getNameInfo("t_bunga_p_k"),

}


const nemali_dega_jamu = {
  'header': getTimingText("2") + "🏆 " + getTranslated("nemali_dega") + " 🏆" + '\n' + getTranslated("nemali_dega")+ ' వంతులో గెలిచే రంగులు ✅',

  "winLakshana": "విన్నింగ్ లక్షణాలు ✅ : లోపల గెట్టి కుసి, పైనా బిరుసు ఉన్నవి గెలుచును.",
  
  "losingLakshana": "లాసింగ్ లక్షణాలు ❌ : లోపల తెల్ల బుంగ ఉండాలి, పైనా నలుపు ఉన్నవి ఓటమి పాలు అవును.",

  '1': getNameInfo("n_d"),

  '2': getNameInfo("nemali_maila"),

  '3': getNameInfo("er_maila"),

}


const kaki_pingali_jamu = {
  'header': getTimingText("1") + "🏆 " + getTranslated("kaki_pingala") + " 🏆" + '\n' + getTranslated("kaki_pingala")+ ' వంతులో గెలిచే రంగులు ✅',

  "winLakshana": "విన్నింగ్ లక్షణాలు ✅ : లోపల నల్ల కుసి ఉండాలి, పైనా మేతదానం ఉన్నవి గెలుచును.",
  
  "losingLakshana": "లాసింగ్ లక్షణాలు ❌ : లోపల గెట్టి కుసి ఉండాలి, పైనా బిరుసు ఉన్నవి ఓటమి పాలు అవును.",

  '1': getNameInfo("kaaaki"),

  '2': getNameInfo("pacha_pettemaru"),

  '3': getNameInfo("pettamari_k"),

  '4': getNameInfo("pacha_kaki"),

  '5': getNameInfo("nbs"),

  '6': getNameInfo("goudu_nemali"),

  '7': getNameInfo("bk"),

}


const dega_pingala_jamu = {
  'header': getTimingText("5") + "🏆 " + getTranslated("dega_pingala") + " 🏆" + '\n' + getTranslated("dega_pingala")+ ' వంతులో గెలిచే రంగులు ✅',

  "winLakshana": "విన్నింగ్ లక్షణాలు ✅ : లోపల గెట్టి కుసి ఉండాలి, పైనా మేతదానం ఉన్నవి గెలుచును.",
  
  "losingLakshana": "లాసింగ్ లక్షణాలు ❌ : లోపల నల్ల బొంగ, పైనా బిరుసు ఉన్నవి ఓటమి పాలు అవును.",

  '1': getNameInfo("org_deg"),

  '2': getNameInfo("erra_kekkirai"),

  '3': getNameInfo("nbs"),
  
  '4': getNameInfo("er_maila"),
  
  '5': getNameInfo("geruva"),


}



const last_jamu = 
{
  'header': getTimingText("6") + "🏆 " + getTranslated("kodi_pingala") + " 🏆" + '\n' + getTranslated("kodi_pingala") + ' వంతులో గెలిచే రంగులు ✅',

  "winLakshana": "విన్నింగ్ లక్షణాలు ✅ : లోపల తెల్ల బుంగ ఉండాలి, పైనా పింగళి చూపు ఉన్నవి గెలుచును.",
  
  "losingLakshana": "లాసింగ్ లక్షణాలు ❌ : లోపల నల్లకుసి ఉండాలి, పైనా బిరుసు ఉన్నవి ఓటమి పాలు అవును.",
  
  '1': getNameInfo("sethuva"),

  '2': getNameInfo("thella_kodi_kekkirayi"),

  '3': getNameInfo("poola_rasangi"),

  '4': getNameInfo("kodi_pingali_2"),

  '5': getNameInfo("kodi_pingali"),

  '6': getNameInfo("kodi_pacha_kaki"),

  '7': getNameInfo("kodi_kaki"),

}


const jsonData = {
    'morning6' : kodi_pingala_jamu,

    'morning8' : kaki_dega_jamu,
  
    'morning10' : nemali_pingala_jamu,
  
    'afternoon1' : kodi_dega_jamu,
  
    'afternoon3' : kaki_nemali_jamu,

    'evening' : last_jamu
  }


  // మే 21 ఆదివారం (ముసుగు పంధాలకి మాత్రమే పని చేసే రంగులు)
  // 6:15am to 8:30am -> (కాకి డేగ) గెలుస్తోంది
  // 8:30am to 10:45am ->  (నెమలి పింగళి)  గెలుస్తోంది
  // 10:45am to 1pm -> (కోడి డేగ) గెలుస్తోంది
  // 1pm to 3:15pm -> (కాకి నెమలి) గెలుస్తోంది
  // 3:15pm to 5:30pm -> (కోడి పింగళి) గెలుస్తోంది
  // 5:30pm నుండి -> (కాకి డేగ) గెలుస్తోంది <-- (రాత్రి పూట పని చేసే రంగులు)


function getTranslated(name) {  
  switch (true) {
    case name.includes("kodi_pingala"):
      return "కోడి పింగళ";
    case name.includes("kaki_dega"):
      return "కాకి డేగ";
    case name.includes("nemali_pingala"):
      return "నెమలి పింగళ";
    case name.includes("kodi_dega"):
      return "కోడి డేగ";
    case name.includes("kodi_kaki"):
      return "కోడి కాకి";
    case name.includes("kodi_nemali"):
      return "కోడి నెమలి";
    case name.includes("kaki_pingala"):
      return "కాకి పింగళ";
    case name.includes("kaki_nemali"):
      return "కాకి నెమలి";
    case name.includes("nemali_dega"):
      return "నెమలి డేగ";
    case name.includes("dega_pingala"):
      return "డేగ పింగళ";
      default:
        return {};
    }
}

function getTimingText(time){
  switch(true){
    case time.includes("1"):
      return "ఉదయం 6:15 నుండి 8:30 గెలిచే రంగు -";
    case time.includes("2"):
      return "ఉదయం 8:30 నుండి 10:45 వరకు గెలిచే రంగు -";
    case time.includes("3"):
      return "ఉదయం 10:45 నుండి మధ్యాహ్నం 1 వరకు గెలిచే రంగు -";
    case time.includes("4"):
      return "మధ్యాహ్నం 1 నుండి మధ్యాహ్నం 3:15 వరకు గెలిచే రంగు -";
    case time.includes("5"):
      return "మధ్యాహ్నం 3:15 నుండి సాయంత్రం 5:30 వరకు గెలిచే రంగు -";
    case time.includes("6"):
      return "సాయంత్రం 5:30 నుండి , మరుసటి రోజు తెల్లవారిన దాక గెలిచే రంగు -";
      default:
        return {};
    }
}




